...
function EditEvent() {
  ...
  useEffect(() => {
    const fetchEventData = async () => {
      try {
        ...
        const response = await eventService.getEventById(id, user.token);
        const data = response.event || response; // Handle both {event: ...} and direct response
        console.log('Fetched event data:', data);
        setEventData(data);

        // Parse time format (e.g., "9:00 AM - 5:00 PM")
        ...
        let endTime = '17:00';
        if (data.eventTime) {
          const times = data.eventTime.split('-').map(t => t.trim());
          if (times.length === 2) {
        ...
        setValue('startDate', formatDateForInput(data.eventDate)); // Backend uses eventDate
        setValue('endDate', formatDateForInput(data.endDate));
        setValue('startTime', startTime);
        setValue('endTime', endTime);
        setValue('location', data.location || '');
        setValue('expectedCapacity', data.expectedCapacity || '');
        setValue('bloodTypesNeeded', data.bloodTypesNeeded || []);
        ...
        // Set location
        if (data.locationCoordinates && data.locationCoordinates.lat && data.locationCoordinates.lng) {
          const locationObj = {
            coordinates: { lat: data.locationCoordinates.lat, lng: data.locationCoordinates.lng },
            address: data.location
          };
          console.log('Setting selectedLocation to:', locationObj);
          setSelectedLocation(locationObj);
        } else {
          console.log('Location coordinates missing - user will need to select location on map');
          // Still set the location text if available
          if (data.location) {
            setValue('location', data.location, { shouldValidate: false });
          }
        }

        // Set eligibility requirements
        if (data.eligibilityRequirements && data.eligibilityRequirements.length > 0) {
  ...
  // Debug: Log selectedLocation changes
  useEffect(() => {
              ...
              <div className="form-group full-width">
                ...
                <div className="map-container">
                  <LocationMap
                    ...
                    center={selectedLocation?.coordinates ? [selectedLocation.coordinates.lat, selectedLocation.coordinates.lng] : [27.7172, 85.324]}
                    zoom={13}
                    showSearch={true}
